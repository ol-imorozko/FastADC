...
public class DCCandidateTrie {

    static int N;

    // TODO: add subtree count
    DCCandidateTrie[] subtrees = new DCCandidateTrie[N];
    DCCandidate dc;

    public DCCandidateTrie() {

    }

    public boolean add(DCCandidate addDC) {
        LongBitSet bitSet = addDC.bitSet;
        DCCandidateTrie treeNode = this;

        for (int i = bitSet.nextSetBit(0); i >= 0; i = bitSet.nextSetBit(i + 1)) {
            DCCandidateTrie[] currSubtrees = treeNode.subtrees;
            treeNode = currSubtrees[i] == null ? (currSubtrees[i] = new DCCandidateTrie()) : currSubtrees[i];
        }
    ...
    private boolean getAndRemoveGeneralizations(LongBitSet superset, int next, List<DCCandidate> removed) {
        ...
        while (nextBit >= 0) {
            DCCandidateTrie subTree = subtrees[nextBit];
            if (subTree != null)
    ...
    private boolean noSubtree() {
        for (DCCandidateTrie subtree : subtrees)
